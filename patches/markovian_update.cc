...
void MarkovianCC::onACK(int ack, 
  ...
  if (unacknowledged_packets.count(seq_num) != 0) {
    ...
    //Changing such that the loop uses an iterator - Avoids Segmentation Fault
    for (auto it = unacknowledged_packets.begin(); it != unacknowledged_packets.end();) {
      ...
      it = unacknowledged_packets.erase(it);

    }

    /**

    //Looping this way does not access things as itertors
    //Should be x.begin() to x.end() as that returns an iterator we can use
    ...
    for (auto x : unacknowledged_packets) {
      ...
      unacknowledged_packets.erase(x.first); //Most likely removing by key instead of iterator
    }
